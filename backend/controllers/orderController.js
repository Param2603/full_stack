...
export const createOrder = async(req, res) => {
    try {
        ...
        const options = { //create an object
            amount: Math.round(Number(amount) * 100), //number (convert string into number) // convert to paise
            currency: currency || "INR",
            receipt: `receipt_${Date.now()}`
        }

        const razorpayOrder = await razorpayInstance.orders.create(options) // method to create razorpay
...
export const getuserOrders = async(req, res) => {
    try {
        ...
        console.log("Enter fetching user order", error)
        res.status(500).json({
            message:error.message
        })
    }
